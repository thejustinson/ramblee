  ...
  useEffect(() => {
    ...
    const fetchFollowing = async () => {
      ...
      const { data, error } = await supabase
        .from("follows")
        .select("following_id, profiles!following_id(handle, display_name, avatar_url)")
        .eq("follower_id", targetUserId);

      if (!error && data) {
        setFollowing(data as unknown as FollowingRow[]);
      }
      setIsLoading(false);
    };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-brand-black/80 backdrop-blur-sm p-4">
      <div className="bg-brand-surface border border-brand-border rounded-[2px] w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        <div className="flex items-center justify-between p-6 border-b border-brand-border">
          <h2 className="font-display text-2xl font-bold text-brand-white">Following</h2>
          <button
            onClick={onClose}
            className="p-2 text-brand-muted hover:text-brand-white transition-colors rounded-[2px]"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-brand-muted">Not following anyone yet.</div>
          ) : (
            <div className="flex flex-col gap-4">
              {following.map((f, i) => (
                <Link
                  key={i}
                  href={`/profile/${f.profiles.handle}`}
                  className="flex items-center gap-4 p-3 hover:bg-brand-black/50 border border-transparent hover:border-brand-border rounded-[2px] transition-colors group"
                >
                  ...
                  <div>
                    ...
                    <div className="font-mono text-xs text-brand-muted">@{f.profiles.handle}</div>
                  </div>
                </Link>
              ))}
            </div>
